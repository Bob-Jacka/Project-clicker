...
export namespace output {
    ...
    */
    namespace colored {
        ...
        template<typename T>
        void
        colored_print(const T &object, const std::string &separator = " ",
                      const std::string &color = Ansi_colors::WHITE) {
            if (std::cout.good()) {
...
//            std::cerr << "Println is failed due to cout" << std::endl;
     ...
     * @tparam T generic type
     * @param str string to output
     * @param print_sep text separator
     */
    template<typename T>
    void print(const T &str, std::string print_sep = "") {
        if (std::cout.good()) {
            std::cout << str << print_sep;
        }
#ifdef LIBIO_ERROR
...
#pragma message("Using libio wide string functionality")
    ...
    template<typename T>
    void
    dynamic_array_output(const T *array, const int size, const bool reverse = false,
...
export namespace string {
    ...
    */
     ...
     */
    std::string change_string_register(const std::string &str, const bool regis) {
        if (regis) {
            return to_lower(str);
        } else {
            return to_upper(str);
